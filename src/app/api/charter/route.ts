...
export async function POST(request: NextRequest) {
  ...
  const { error } = await resend.emails.send({
    ...
    subject: "New Charter Member Application",
    html: `
  ...
  try {
    ...
    await dynamo.send(new PutCommand({ TableName: CHARTER_TABLE, Item: item }));
  } catch (dynamoError) {
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ charter: items }, { status: 200 });
  } catch (dynamoError) {
    console.error("DynamoDB Scan failed:", dynamoError);
    return NextResponse.json({ error: "Failed to fetch charter members" }, { status: 500 });
  }
}
